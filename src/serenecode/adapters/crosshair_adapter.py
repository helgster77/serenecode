...
@icontract.require(
    lambda all_checkables: isinstance(all_checkables, list),
    "all_checkables must be a list",
)
@icontract.require(
...
@icontract.ensure(
    lambda all_checkables, result: isinstance(result, list)
    and len(result) <= len(all_checkables),
    "result must be a subset-sized list of all_checkables",
)
def _filter_autogenerated(
    all_checkables: list[Any],
    findings: list[SymbolicFinding],
    module_path: str,
) -> list[Any]:
...
@icontract.require(
    lambda checkables: isinstance(checkables, list),
    "checkables must be a list",
)
@icontract.require(
    lambda findings: isinstance(findings, list),
    "findings must be a list",
...
@icontract.ensure(
    ...
    "function mutates the findings list in place",
)
